...
def main(*args):
    ...
    for read in nodes:
        ...
        paint_task= st.task.find("item_id=%s and project_id=%s and code=paint"%(shot_id,project_id))
        if paint_task:
            paint_task_id=paint_task.get("id")
            st.task.update(id=paint_task_id,fields={"assignee":user_id})
            continue
        data = {"approved_version": 001, "assignee": user_id, "cc_id": "", "code": "paint", "current_version": 001,
                "delivered_version": 001, "due_date": "", "item_id": shot_id, "json": "", "level": "", "lock": 0,
                "milestone": 0, "name": "paint", "priority": 30, "project_id": project_id, "status_id": 24, "step_id": 41,
                "sub_date": "", "tag_ids": ""}
        st.task.create(data)
    nuke.message("任务创建完成")