...
def main():
    ...
    if seq_id:
        ...
        epi_id = StrackGlobals.st.sequence.find("id={}".format(seq_id), ["episode_id"]).get("episode_id")
        if epi_id:
            epi_name = StrackGlobals.st.episode.find("id=%s" % epi_id).get("code")
    else:
        seq_name = ""
        epi_name = ""
    nuke_path = os.path.join(root_dir, project_name,epi_name, "sequences", seq_name, shot_name, "work", step_name, task_code, "nuke")
    ...
    render_dir = os.path.join(root_dir, project_name,epi_name, "sequences", seq_name, shot_name, "work", step_name,task_code, "renders")
